...
class RaceTest < ActiveSupport::TestCase
  ...
  test "should create valid race with default values for pre-race setup" do
    race = @edmonton_bmx.races.build
    assert race.valid?
    assert_equal 0, race.at_gate
    assert_equal 0, race.in_staging
    assert_equal true, race.active
  end
  ...
  test "should validate at_gate moto number is non-negative" do
    race = @edmonton_bmx.races.build(at_gate: -1, in_staging: 0)
    assert_not race.valid?
  ...
  test "should validate in_staging moto number is non-negative" do
    race = @edmonton_bmx.races.build(at_gate: 0, in_staging: -1)
    assert_not race.valid?
  ...
  test "should validate moto at gate is less than moto in staging during active racing" do
    race = @edmonton_bmx.races.build(at_gate: 15, in_staging: 12)
    assert_not race.valid?
  ...
  test "should allow both counters at zero before racing begins" do
    race = @edmonton_bmx.races.build(at_gate: 0, in_staging: 0)
    assert race.valid?
  end
  ...
  test "should allow valid moto progression scenarios" do
    ...
    valid_moto_combinations.each do |at_gate, in_staging|
      race = @edmonton_bmx.races.build(at_gate: at_gate, in_staging: in_staging)
  ...
  test "should have active scope for current race day" do
    todays_race = @edmonton_bmx.races.create!(at_gate: 0, in_staging: 3, active: true)
    practice_race = @edmonton_bmx.races.create!(at_gate: 0, in_staging: 1, active: false)
    
    assert_includes Race.active, todays_race
    assert_not_includes Race.active, practice_race
  end
  ...
  test "should have one race_setting with dependent destroy for race day configuration" do
    ...
    race = calgary_olympic.races.create!(at_gate: 0, in_staging: 1)
  ...
  test "should belong to organizing club" do
    race = @edmonton_bmx.races.create!(at_gate: 0, in_staging: 1)
